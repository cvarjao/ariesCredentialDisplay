...
export interface OCACredentialBundle {
  ...
  getOverlay<F extends BaseOverlay>(
    type: string,
    language?: string,
  ): F | undefined;
...
export interface OCABundleResolver {
  getCredentialPresentationFields(
...
export class DefaultOCACredentialBundle implements OCACredentialBundle {
  ...
  public getCardLayoutOverlay(): CardLayoutOverlay | undefined {
    const layout = this.getOverlay<CardLayoutOverlay>(
      OverlayType.CARD_LAYOUT_10,
    );
    return layout;
  }
  ...
  public getOverlay<F extends BaseOverlay>(
    type: string,
    language?: string,
  ): F | undefined {
    ...
    if (language !== undefined) {
      return (this.bundle as Bundle).overlays.find(
        item =>
          ...
          (item as BaseL10nOverlay).language === language,
      ) as F;
    }
    return (this.bundle as Bundle).overlays.find(
      item => item.type === type.toString(),
    ) as F;
  }
}
    ...
    for (const item of [credentialDefinitionId, schemaId]) {
      if (item && this.bundles[item] !== undefined) {
        ...
        return Promise.resolve(
          new DefaultOCACredentialBundle(bundle as Bundle),
        );
      }
    }
  ...
  public async getCredentialPresentationFields(
    ...
    if (baseOverlay && baseOverlay.attributes) {
      for (const key in baseOverlay?.attributes) {
        if (
          Object.prototype.hasOwnProperty.call(baseOverlay?.attributes, key)
        ) {
          const _sourceField = credential?.credentialAttributes?.find(
            item => item.name === key,
          );
          if (_sourceField) {
            const _field: Field = {
            ...
            _field.encoding =
              characterEncodingOverlay?.attr_character_encoding[key];
      ...
      if (credential.credentialAttributes) {
        for (const _sourceField of credential.credentialAttributes) {
          ...
          _field.encoding =
            characterEncodingOverlay?.attr_character_encoding[
              _sourceField.name
            ];
